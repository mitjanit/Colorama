...
class CosineGradient {
	...
	void displayGradientInv(PVector orig, float w, float h){
			
		float wp = w/(float)this.num;
		int n = this.values.length-1;
		noStroke();
		for(int i=0; i<this.num; i++){
			float redValue = red(this.values[n-i]);
			float greenValue = green(this.values[n-i]);
			float blueValue = blue(this.values[n-i]);
			fill(redValue,greenValue, blueValue);
			rect(orig.x + i*wp, orig.y, wp, h);
		}
	}
